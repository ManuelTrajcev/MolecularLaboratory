...
- The info panel can also be toggled with keyboard **Y** in desktop play; XR/simulator binding remains left-controller Y / `Left Shift + 2`. Its controls column is mode-aware: active `MouseControlCamera` shows desktop mouse/keyboard controls, otherwise it shows XR/simulator controls.
...
- `Assets/Scenes/Laboratory - new models.unity` currently has the XR Origin prefab instance disabled and a standalone `MouseControlCamera` GameObject enabled/tagged `MainCamera`. The camera has `MouseControlCamera` (`moveSpeed=3`, `lookSensitivity=0.2`, `reachDistance=20`, `heldKeyboardMoveSpeed=3`), a Camera, AudioListener, and URP additional camera data, positioned at `(0, 1.1, -2.24)` with identity rotation. Treat this scene as the current desktop/mouse test path; XR headset testing still needs the XR Origin re-enabled or a dedicated XR scene variant.
- `MouseControlCamera.cs` now creates a hidden runtime `XRRayInteractor` and an `XR Interaction Manager` if needed, then uses XRI manual selection instead of invoking select events directly. PC controls: locked-FPS mouse look (`Esc` unlocks, left click re-locks), `WASD` move, `Space`/`C` vertical, `Left Shift` sprint when not holding anything, `Left Ctrl` precision, `LMB` interact/grab, mouse wheel held-object depth, `Shift+WASD` moves the held atom/molecule without moving the camera, `R`+mouse held-object/molecule rotation, `X` delete aimed atom, `Z` zoom.
