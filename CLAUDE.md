...
- The info panel can also be toggled with keyboard **Y** in desktop play; XR/simulator binding remains left-controller Y / `Left Shift + 2`. Its controls column is mode-aware: active `MouseControlCamera` shows desktop mouse/keyboard controls, otherwise it shows XR/simulator controls. `LevelManager` also clamps XR simulator/player camera height to Y=0.6-2.4 so Q/E vertical movement cannot leave the lab height range.
