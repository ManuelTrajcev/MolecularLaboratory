...
- Main Menu instructions and the in-lab info panel now share `LaboratoryInstructionContent`, so gameplay text and XR/PC controls stay identical; the Main Menu panel rebuilds at runtime into the same dark title/welcome/two-column design as the lab info panel. The info panel can also be toggled with keyboard **Y** in desktop play; XR/simulator binding remains left-controller Y / `Left Shift + 2`. Its controls column is mode-aware: active `MouseControlCamera` shows desktop mouse/keyboard controls, otherwise it shows XR/simulator controls. In mouse-camera mode, Main Menu XRI button events require a precise camera ray hit before invoking the button, preventing broad desktop sphere-cast near misses from accidentally triggering Exit. `LevelManager` also clamps XR simulator/player camera height to Y=0.6-2.4 so Q/E vertical movement cannot leave the lab height range.
