...
The auto-instantiated `XR Interaction Simulator` sample prefab has its `m_InteractionSimulatorUI` reference cleared, so simulator input still works but the on-screen key-mapping/Input Selection overlay is hidden in both mouse-camera and XR scenes.
`SceneFadeManager.LoadScene(...)` now provides a black fade-out/fade-in transition across scene loads; its runtime fade quad is detached to the persistent manager before loading and reattached to the new scene camera before fade-in, so it survives transitions between Main Menu and Laboratory.
...
- `MouseControlCamera.cs` now creates a hidden runtime `XRRayInteractor` and an `XR Interaction Manager` if needed, then uses XRI manual selection instead of invoking select events directly. PC controls: locked-FPS mouse look, `WASD` move, `Space`/`C` vertical clamped to Y=0.6-2.4, `Left Shift` sprint when not holding anything, `Left Ctrl` precision, `LMB` interact/grab, mouse wheel held-object depth, `Shift+WASD` moves the held atom/molecule without moving the camera, `X` delete aimed atom, `Z` zoom to 18 FOV.
...
**WebGL build support / Meta XR SDK swap (2026-06-30):**
- `Packages/manifest.json` now lists `com.meta.xr.sdk.core` (v201.0.0) as a **direct dependency** — despite the original "pure OpenXR, no Meta SDK" intent. No project script references it (no `using Meta`/`OVR`/`Meta.XR` in `Assets/Scripts/`); it appears to be an unused/experimental add.
- **The Meta XR Core SDK does not compile for WebGL.** Its `Meta.XR.BuildingBlocks.AIBlocks` assembly (`SpeechToTextAgent.cs`) references `UnityEngine.Microphone`, which Unity strips on WebGL → `CS0103` → build fails with 5 errors. All other Meta assemblies (incl. `Oculus.VR`) *do* compile for WebGL; native plugins are import-gated to Win64/Android so they're skipped. A second latent offender (`LipSyncMicInput.cs` in `MultiplayerBlocks.Shared`) is gated behind Photon/Avatar define constraints, so it doesn't bite today. Meta's asmdefs use `includePlatforms/excludePlatforms: []` (all platforms), so they are NOT gated away from WebGL by the vendor.
- WebGL also can't run the OpenXR VR runtime — a web build of this project is a **flat, non-VR** experience (mouse/keyboard via `MouseControlCamera`), not a headset build.
- **Build-time swap tooling:** `Assets/Scripts/Editor/MetaSdkWebBuildToggle.cs` (namespace `MolecularLab.EditorTools`) adds a **Tools ▸ Web Build** menu with *Remove Meta SDK (for WebGL)* and *Restore Meta SDK (for Quest)*. Uses `PackageManager.Client.Remove`/`Add` (edits the manifest + triggers resolve/recompile; never hand-edits `manifest.json` text). Workflow: Remove → **wait for recompile** → build WebGL → Restore. A build *pre-processor* can't do this automatically — package add/remove recompiles asynchronously and script compilation happens before any build preprocess callback, so the swap must be a manual two-step around the build.
- **Note:** the Unity MCP server in this workspace is currently connected to a *different* project (`/Users/manuel/Documents/PROJECTS/UnityProjects/MatrixShift/`), not Molecular Laboratory — so `Unity_GetConsoleLogs` / `Unity_RunCommand` etc. operate on MatrixShift, not this project. Verify which Editor the MCP is attached to before trusting MCP console output for this repo.
