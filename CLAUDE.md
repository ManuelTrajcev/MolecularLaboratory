...
**Molecule-guidance info button restyle (2026-06-17):** in `LevelManager.cs`, the procedurally-built info ("i") button on the world-space `MoleculeGuidancePrompt` canvas was moved to sit **just outside the prompt's left edge** (anchor/pivot top-left of the prompt, top-aligned, offset left by `infoButtonSize.x + infoButtonPadding` so it clears the popup; `infoButtonPadding` new field, default 18, doubles as the gap width), enlarged (`infoButtonSize` 70→110), and given a **circular** background via Unity's built-in `UI/Skin/Knob.psd` sprite (cached in static `_circleSprite`, fetched with `Resources.GetBuiltinResource`). `infoButtonColor` alpha 0.96→0.5 for a **half-transparent light-blue** disc; the "i" glyph stays black. Size/padding/tint are all `[SerializeField]`-tunable.
